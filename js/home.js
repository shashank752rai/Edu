...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function initHeroSlider() {
        ...
        try {
            const response = await fetch('admin/api/get_banners.php');
            ...
            if (banners.length > 0) {
                ...
                // Create slides from admin banners
                banners.forEach((banner, index) => {
                    const slide = document.createElement('div');
                    slide.className = 'hero-slide';
                    if (index === 0) slide.classList.add('active');
                    
                    slide.innerHTML = `<img src="${banner.image_path}" alt="${banner.title}" />`;
                    heroSlider.appendChild(slide);
                });
                
                slides = document.querySelectorAll('.hero-slide');
            } else {
                // Fallback to existing slides if no admin banners
                slides = document.querySelectorAll('.hero-slide');
            }
        } catch (error) {
            console.error('Error loading banners:', error);
            // Use existing slides as fallback
            slides = document.querySelectorAll('.hero-slide');
        }
